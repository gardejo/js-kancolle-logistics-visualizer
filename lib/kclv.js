...
/**
 * Create a new relation array with all elements that pass the test implemented
 *     by the specified tester function.
 * @public
 * @const
 * @param {Function} tester A function that test each tuple of the relation.
 * @param {Object=} opt_thisObject Value to use as {@code this} when executing
 *     {@code tester} function.
 * @return {Array.<Array>} A filtered new relation.
 * @nosideeffects
 * @see Array#filter()
 */
kclv.Relation.Base.prototype.filter = function(tester, opt_thisObject) {
    return this.relation.filter(tester, opt_thisObject);
};
...
kclv.Table.Materials.Line.prototype.buildRows = function() {
    ...
    return this.relation.clone().
        ...
        filter( function(tuple) {
            // tuple[0] was always filled with DateTime.
            return ! tuple.slice(1).every( function(attribute) {
...
kclv.Table.Ships.Bubble.prototype.buildRoster_ = function() {
    ...
    this.relation.forEach( function(tuple) {
        ...
        if (mothballLevel && level <= mothballLevel) {
            return;
        }
...
kclv.Chart.Bubble.prototype.buildOption = function() {
    ...
    option.vertical.maximum = this.getThreshold(
        configuration, 'vertical', 'maximum', 'AverageLevel'
    );
    option.vertical.minimum = this.getThreshold(
        configuration, 'vertical', 'minimum', 'AverageLevel'
    );
    ...
    if (configuration.horizontal) {
        option.horizontal.maximum = this.getThreshold(
            configuration, 'horizontal', 'maximum', 'TotalShipNumber'
        );
        option.horizontal.minimum = this.getThreshold(
            configuration, 'horizontal', 'minimum', 'TotalShipNumber'
        );
