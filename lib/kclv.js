...
kclv.Stream.prototype.readFile = function(path, opt_characterSet) {
    try {
        ...
        var contents = stream.ReadText();
            // default ADODB.StreamReadEnum.adReadAll
...
kclv.Table.Materials.Line = function(relation, kind) {
    ...
    this.hasOpposite = kind === 'Resources' ?
        kclv.Configuration.get('chart.Resources.withRepair') : false;
...
kclv.Table.Ships.Base.prototype.buildColumns = function() {
    var legend = kclv.Configuration.get(['legend', this.locale, 'Ships']),
        label = kclv.Configuration.get('chart.Ships.abbreviate') ?
            ...
            f : tuple[1] + ' Lv.' + tuple[3] + (
                    this.kind === 'Levels' ?
                        ...
                        ' ' + this.formatter.parenthesize(
                            this.formatter.commify(tuple[attribute])
                        )
                )
        };
...
kclv.Chart.Base.prototype.buildOption = function(opt_configuration) {
    ...
    option.redraw = kclv.Configuration.get('chart.redraw');
    option.vertical = {};
    option.horizontal = {};
    ...
    option.vertical.gridlines = opt_configuration.vertical ?
        opt_configuration.vertical.gridlines : null;
    option.vertical.minorGridlines = opt_configuration.vertical ?
        opt_configuration.vertical.minorGridlines : null;

    option.horizontal.gridlines = opt_configuration.horizontal ?
        opt_configuration.horizontal.gridlines : null;
    option.horizontal.minorGridlines = opt_configuration.horizontal ?
        opt_configuration.horizontal.minorGridlines : null;
...
kclv.Chart.Candlestick.prototype.buildOption = function() {
    ...
    option.hollowIsRising = kclv.Configuration.get('chart.hollowIsRising');
...
/**
 * Build a title of the histogram chart and get it.
 *     TODO: Refactor it.
 * @protected
 * @override
 * @returns {string} A title of the histogram chart.
 */
kclv.Chart.Histogram.prototype.buildTitle = function() {
    var configuration = kclv.Configuration.get([
        'legend', this.table.locale, 'Ships'
    ]);
...
/**
 * Build a title of the scatter chart and get it.
 *     TODO: Refactor it.
 * @protected
 * @override
 * @returns {string} A title of the scatter chart.
 */
kclv.Chart.Scatter.prototype.buildTitle = function() {
    var configuration = kclv.Configuration.get([
        'legend', this.table.locale, 'Ships'
    ]);
...
kclv.Chart.Scatter.prototype.buildOption = function() {
    ...
    var step = configuration.vertical.step;
...
kclv.Visualizer.prototype.visualize = function(directive) {
    ...
    var path = kclv.Configuration.get('chart.path.chart') + '/' +
               [
                   directive.agent,
                   directive.relation,
                   directive.chart,
