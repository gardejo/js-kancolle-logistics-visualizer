    ...
    return {
        ...
        getInstance : function(namespace, name, opt_arguments) {
            var nodes = name ?
                ...
                constructor = nodes.reduce( function(
...
kclv.Tokenizer.Logbook.Materials.prototype.toColumns = function(
    ...
    var columns = kclv.Tokenizer.Logbook.Base.prototype.toColumns.apply(
            this, arguments
        ),
        // Convert "2013-04-23 01:23:45" into "2013/04/23 01:23:45".
        dateTime = new Date( columns.shift().replace(/-/g, '/' ) );
...
kclv.Table.Materials.Candlestick.prototype.buildRows = function() {
    ...
    table.push( this.relation.reduce( function(row, tuple, index, relation) {
        var period = row[0],
            current = new kclv.Date(tuple[0]).toPeriod(frequency),
            value = tuple[attribute];

        if ( period === current ) { // Staying the same period.
            ...
            if (index) { // Not the first row. (row !== [])
...
kclv.Table.Ships.Base.prototype.buildRows = function() {
    ...
    if (this.option === 'Experiences') {
        relation.sort( function(a, b) {
            return b[4] - a[4]; // Compare experience pionts descendingly.
        } );
    }

    return relation.map( function(tuple, index, relation) {
        var columns = [];
...
kclv.Chart.Base = function(name, table) {
     ...
     * @const
     */
    this.title = this.table.buildTitle();
