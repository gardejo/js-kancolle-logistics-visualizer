...
kclv.Agent.Base.prototype.buildRelation = function(relationName) {
    var relation = kclv.Factory.getInstance(kclv.Relation, relationName).
                   insert( this.buildRelationalArray(relationName) );
 ...
 * @nosideeffects
 */
kclv.Agent.Base.prototype.buildRelationalArray = function(
 ...
 *     {@code kclv.Tokenizer.Manual.Ships.prototype.toRelationalArray} method,
 ...
 *     log file.
 */
kclv.Agent.Manual.prototype.buildRelationalArray = function(relationName) {
    var relation = kclv.Agent.Base.prototype.buildRelationalArray.call(
            this, relationName
        ),
        EXPERIENCES = kclv.Game.Ships.EXPERIENCES;

    if (relationName === 'Ships') {
        relation = relation.map( function(columns, index) {
            return [
...
kclv.Agent.SandanshikiKanpan.prototype.buildRelation = function(relationName) {
    if (relationName !== 'Materials') {
        throw new Error(
            'Sandanshiki Kanpan does not dump (' + relationName + ').'
        );
    }
    ...
    MATERIALS.forEach( function(material, index) {
        ...
        this.buildRelationalArray(relationName, material).forEach( function(
            tuple
        ) {
