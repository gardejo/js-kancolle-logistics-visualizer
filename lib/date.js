...
kclv.Date.prototype.toPeriod = function(frequency) {
    switch (frequency) {
        case 'Yearly':
            return {
                v : new Date( this.date_.getFullYear(), 0, 1 ),
                f : this.date_.getFullYear().toString()
            };
        case 'Monthly':
            return {
                v : new Date(
                        this.date_.getFullYear(), this.date_.getMonth(), 1
                    ),
            ...
            return {
                v : new Date(
                        new Date(
                            ...
                            this.date_.getMonth(),
                            this.date_.getDate()
                        ).setDate( this.date_.getDate() - (
                            // Someday to Monday of the week.
                            this.date_.getDay() ? this.date_.getDay() - 1 : 6
                        ) )
                    ),
                f : this.toWeekString()
            };
