...
kclv.Chart.Base.prototype.buildHorizontalOption = function(
    ...
    this.setThreshold(option, configuration, 'horizontal', opt_kind);
 ...
 * @return {Object|undefined} A value which set by the method.
 */
...
kclv.Chart.Base.prototype.buildChronologicalTicks_ = function(option, step) {
    // Get the first tick.
    var ticks = [ new kclv.Date(option.minimum).toPeriod(step).v ];

    // Add ticks gradually.
    while ( ticks[ticks.length - 1] < option.maximum ) {
        ticks.push(
            new kclv.Date( ticks[ticks.length - 1] ).toNextPeriod(step)
        );
    }
